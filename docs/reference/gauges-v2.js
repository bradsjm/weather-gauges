...
gauges = (function () {
  ...
  var strings = LANG.EN, // Set to your default language. Store all the strings in one object
    ...
    ajaxDelay = config.longPoll
      ? Math.max(config.realtimeInterval - 20, 0)
    ...
    init = function (dashboard) {
      ...
      switch (config.weatherProgram) {
        case 0:
          ...
          config.realTimeURL = config.longPoll
            ? config.realTimeUrlLongPoll
            : config.realTimeUrlCumulus
          ...
          config.tipImgs = [
            // config.tipImgs for Cumulus users using the 'default' weather site
        ...
        case 3:
          ...
          config.tipImgs = [
            // config.tipImgs - WeatherCat users using the 'default' weather site
        ...
        case 5:
          ...
          config.showCloudGauge = false
          config.tipImgs = [
            // config.tipImgs for WView users
    ...
    singleLed = (function () {
      ...
      function init() {
        ...
        if ($('#canvas_led').length) {
          led = new steelseries.Led('canvas_led', {
    ...
    singleStatus = (function () {
      ...
      function init() {
        ...
        if ($('#canvas_status').length) {
          scroller = new steelseries.DisplaySingle('canvas_status', {
    ...
    singleTimer = (function () {
      ...
      function init() {
        ...
        if ($('#canvas_timer').length) {
          lcd = new steelseries.DisplaySingle('canvas_timer', {
    ...
    singleTemp = (function () {
      ...
      function init() {
        ...
        function update() {
          ...
          cache.minValue =
            data.tempunit[1] === 'C' ? gaugeGlobals.tempScaleDefMinC : gaugeGlobals.tempScaleDefMinF
          cache.maxValue =
            data.tempunit[1] === 'C' ? gaugeGlobals.tempScaleDefMaxC : gaugeGlobals.tempScaleDefMaxF
          ...
          if (cache.selected !== sel) {
            ...
            if (config.showPopupGraphs && config.tipImgs[0][cache.popupImg] !== null) {
              ...
              $('#imgtip0_img').attr(
                'src',
                config.imgPathURL + config.tipImgs[0][cache.popupImg] + cacheDefeat
              )
            }
          }
          ...
          if (
            cache.minValue !== ssGauge.getMinValue() ||
          ...
          if (ddimgtooltip.showTips) {
            ...
            if (cache.selected === 'out') {
              ...
              if (cache.trendVal !== -9999) {
                tip +=
                  '<br>' +
                  strings.temp_trend_info +
                  ': ' +
                  tempTrend(cache.trendVal, data.tempunit, true) +
                  ' ' +
                  cache.trendVal +
                  data.tempunit +
                  '/h'
        ...
        function update() {
          ...
          if (cache.selected !== sel) {
            ...
            if (config.showPopupGraphs && config.tipImgs[1][cache.popupImg] !== null) {
              ...
              $('#imgtip1_img').attr(
                'src',
                config.imgPathURL + config.tipImgs[1][cache.popupImg] + cacheDefeat
              )
            }
          }
          ...
          if (data.rainunit === 'mm') {
            ...
            if (cache.value <= 1) {
              cache.maxValue = Math.max(
                nextHighest(cache.value, 0.5),
                gaugeGlobals.rainScaleDefMaxIn
              )
        ...
        function update() {
          ...
          if (ddimgtooltip.showTips) {
            ...
            tip =
              ...
              data.TrrateTM +
              ' | ' +
              strings.max_hour_info +
              ': ' +
              extractDecimal(data.hourlyrainTH) +
              ' ' +
              data.rainunit +
              ' ' +
              strings.at +
        ...
        function update() {
          ...
          if (cache.selected !== sel) {
            ...
            if (config.showPopupGraphs && config.tipImgs[4][cache.popupImg] !== null) {
              ...
              $('#imgtip4_img').attr(
                'src',
                config.imgPathURL + config.tipImgs[4][cache.popupImg] + cacheDefeat
              )
            }
          }
    ...
    singleBaro = (function () {
      ...
      function init() {
        ...
        function update() {
          ...
          cache.trendVal =
            +extractDecimal(data.presstrendval) / (config.weatherProgram === 2 ? 3 : 1)

          if (data.pressunit === 'hPa' || data.pressunit === 'mb') {
            //  default min range 990-1030 - steps of 10 hPa
            cache.minValue = Math.min(
              nextLowest(cache.recLow - 2, 10),
              gaugeGlobals.baroScaleDefMinhPa
            )
            cache.maxValue = Math.max(
              nextHighest(cache.recHigh + 2, 10),
              gaugeGlobals.baroScaleDefMaxhPa
            )
            dps = 1 // 1 decimal place
          } else if (data.pressunit === 'kPa') {
            //  default min range 99-105 - steps of 1 kPa
            cache.minValue = Math.min(
              nextLowest(cache.recLow - 0.2, 1),
              gaugeGlobals.baroScaleDefMinkPa
            )
            cache.maxValue = Math.max(
              nextHighest(cache.recHigh + 0.2, 1),
              gaugeGlobals.baroScaleDefMaxkPa
            )
            dps = 2
          } else {
            // inHg: default min range 29.5-30.5 - steps of 0.5 inHg
            cache.minValue = Math.min(
              nextLowest(cache.recLow - 0.1, 0.5),
              gaugeGlobals.baroScaleDefMininHg
            )
            cache.maxValue = Math.max(
              nextHighest(cache.recHigh + 0.1, 0.5),
              gaugeGlobals.baroScaleDefMaxinHg
            )
            dps = 3
          }
          ...
          if (ddimgtooltip.showTips) {
            ...
            if (cache.trendVal !== -9999) {
              tip +=
                '<br>' +
                '- ' +
                strings.baro_trend_info +
                ': ' +
                baroTrend(cache.trendVal, data.pressunit, true) +
                ' ' +
                (cache.trendValRnd > 0 ? '+' : '') +
                cache.trendValRnd +
                ' ' +
      ...
      function init() {
        ...
        function update() {
          ...
          switch (data.windunit) {
            ...
            case 'kts':
              cache.maxValue = Math.max(
                nextHighest(cache.maxGustToday, 10),
                gaugeGlobals.windScaleDefMaxMph
              )
              break
            case 'm/s':
              cache.maxValue = Math.max(
                nextHighest(cache.maxGustToday, 5),
                gaugeGlobals.windScaleDefMaxMs
              )
              break
            default:
              cache.maxValue = Math.max(
                nextHighest(cache.maxGustToday, 20),
                gaugeGlobals.windScaleDefMaxKmh
              )
          }
          cache.areas = [
          ...
          if (ddimgtooltip.showTips) {
            // update tooltip
            tip =
              strings.tenminavgwind_info +
              ': ' +
              cache.average +
              ' ' +
              data.windunit +
              ' | ' +
              strings.maxavgwind_info +
              ': ' +
              cache.maxAvgToday +
              ' ' +
              data.windunit +
              '<br>' +
              strings.tenmingust_info +
              ': ' +
              cache.gust +
              ' ' +
              data.windunit +
              ' | ' +
              strings.maxgust_info +
              ': ' +
              cache.maxGustToday +
              ' ' +
              data.windunit +
        ...
        function update() {
          ...
          if (config.showWindVariation) {
            ...
            if (config.showWindMetar) {
              ssGauge.VRB =
                ' - METAR: ' +
                ('0' + data.avgbearing).slice(-3) +
                ('0' + cache.avgKnots).slice(-2) +
                'G' +
                ('0' + cache.gstKnots).slice(-2) +
                'KT '
            } else {
              ssGauge.VRB = ''
            }
            if (windSpd > 0) {
              ...
              range =
                (+cache.bearingTo < +cache.bearingFrom
                  ? 360 + +cache.bearingTo
                  : +cache.bearingTo) - +cache.bearingFrom

              if (cache.avgKnots < 3) {
                // Europe uses 3kts, USA 6kts as the threshold
                if (config.showRoseOnDirGauge) {
                  ssGauge.setSection([
                    steelseries.Section(
                      cache.bearingFrom,
                      cache.bearingTo,
                      gaugeGlobals.windVariationSector
                    )
                  ])
                  ssGauge.setSection([])
                } else {
                  ssGauge.setSection([
                    steelseries.Section(cache.bearingFrom, cache.bearingTo, gaugeGlobals.minMaxArea)
                  ])
                  ssGauge.setArea([])
                }
              ...
              if (config.showWindMetar) {
                if (
                  (range < 60 && range > 0) ||
                  (range === 0 && cache.bearingFrom === cache.valueAverage)
                ) {
                  ssGauge.VRB += ' STDY'
            ...
            if (roseMax > 0) {
              ...
              for (i = 0; i < rosePoints; i++) {
                roseAreas[i] = steelseries.Section(
                  ...
                  'rgba(' +
                    gradient('2020D0', 'D04040', data.WindRoseData[i] / roseMax) +
                    ',' +
          ...
          if (ddimgtooltip.showTips) {
            // update tooltip
            tip =
              strings.latest_title +
              ' ' +
              strings.bearing_info +
              ': ' +
              cache.valueLatest +
              '° (' +
              getord(+cache.valueLatest) +
              ')' +
              ssGauge.VRB +
              '<br>' +
              strings.tenminavg_web +
              ' ' +
              strings.bearing_info +
              ': ' +
              cache.valueAverage +
              '° (' +
              getord(+cache.valueAverage) +
              '), ' +
              strings.dominant_bearing +
              ': ' +
              data.domwinddir
            if (!config.showRoseGauge) {
              // Wind run is shown on the wind rose if it is available
              tip +=
                '<br>' + strings.windruntoday + ': ' + data.windrun + ' ' + displayUnits.windrun
            }
            ...
            if (ddimgtooltip.showTips) {
              $('#imgtip10_txt').html(
                strings.dominant_bearing +
                  ...
                  data.domwinddir +
                  '<br>' +
                  strings.windruntoday +
                  ': ' +
                  data.windrun +
                  ' ' +
                  displayUnits.windrun
              )
            }
          }
        } // End of update()
    ...
    singleUV = (function () {
      ...
      function init() {
        ...
        cache.gradient = new steelseries.gradientWrapper(
          0,
          16,
    ...
    singleSolar = (function () {
      ...
      function init() {
        ...
        function update() {
          ...
          percent =
            +cache.currMaxValue === 0
              ? '--'
              : Math.round((+cache.value / +cache.currMaxValue) * 100)
          ...
          if (data.CurrentSolarMax !== 'N/A') {
            ssGauge.setArea([
              ...
              steelseries.Section(
                Math.max(
                  (cache.currMaxValue * gaugeGlobals.sunshineThresholdPct) / 100,
                  gaugeGlobals.sunshineThreshold
                ),
                cache.currMaxValue,
                'rgba(255,255,50,0.4)'
              ),
      ...
      function init() {
        ...
        function update() {
          ...
          if (data.cloudbaseunit === 'm') {
            ...
            cache.maxValue = Math.max(
              nextHighest(cache.value, 1000),
              gaugeGlobals.cloudScaleDefMaxm,
              cache.maxValue
            )
            ...
            cache.maxValue = Math.max(
              nextHighest(cache.value, 2000),
              gaugeGlobals.cloudScaleDefMaxft,
              cache.maxValue
            )
      ...
      if (typeof data.ver !== 'undefined' && data.ver >= realtimeVer) {
        ...
        try {
          ...
          if (then.getTime() >= today.getTime()) {
            ...
            data.LastRained =
              then.getDate().toString() +
              ' ' +
              strings.months[then.getMonth()] +
              ' ' +
              strings.at +
              ' ' +
              str[1]
        ...
        if (now - sampleDate > config.stationTimeout * 60 * 1000) {
          ...
          ledIndicator.setTitle(
            strings.led_title_ok + '. ' + strings.StatusLastUpdate + ': ' + data.date
          )
        ...
        if (
          config.showCloudGauge &&
          (config.weatherProgram === 4 || config.weatherProgram === 5 || data.cloudbasevalue === '')
        ) {
          ...
          data.cloudbasevalue = calcCloudbase(
            data.temp,
            data.tempunit,
            data.dew,
            data.cloudbaseunit
        ...
        if (data.ver < realtimeVer) {
          ...
          statusScroller.setText(
            'Your ' +
              config.realTimeURL.substr(config.realTimeURL.lastIndexOf('/') + 1) +
              ' file template needs updating!'
          )
          return false
        } else {
    ...
    pageTimeout = function () {
      ...
      $('#canvas_led').click(function click() {
        ...
        tickTockInterval = setInterval(function tick() {
          $.publish('gauges.clockTick', null)
        }, 1000)
    ...
    doFirst = function () {
      ...
      if (config.showPopupData && config.showPopupGraphs) {
        ...
        if (gaugeDew) {
          $('#imgtip1_img').attr(
            'src',
            config.imgPathURL + config.tipImgs[1][gaugeDew.data.popupImg] + cacheDefeat
          )
        }
        $('#imgtip2_img').attr('src', config.imgPathURL + config.tipImgs[2] + cacheDefeat)
        $('#imgtip3_img').attr('src', config.imgPathURL + config.tipImgs[3] + cacheDefeat)
    ...
    nmiles2km = function (val) {
      return (extractDecimal(val) * 1.852).toFixed(1)
    },
    // km to miles
    ...
    setRainUnits = function (mm) {
      if (mm) {
        ...
        if (gaugeRain) {
          ...
          gaugeRain.data.sections = gaugeGlobals.rainUseSectionColours
            ? createRainfallSections(true)
            : []
          gaugeRain.data.maxValue = gaugeGlobals.rainScaleDefMaxmm
          gaugeRain.data.grad = gaugeGlobals.rainUseGradientColours
            ? createRainfallGradient(true)
            : null
        }
        if (gaugeRRate) {
        ...
        if (gaugeRain) {
          ...
          gaugeRain.data.sections = gaugeGlobals.rainUseSectionColours
            ? createRainfallSections(false)
            : []
          gaugeRain.data.maxValue = gaugeGlobals.rainScaleDefMaxIn
          gaugeRain.data.grad = gaugeGlobals.rainUseGradientColours
            ? createRainfallGradient(false)
            : null
        }
        if (gaugeRRate) {
    ...
    gradient = function (startCol, endCol, fraction) {
      var redOrigin, grnOrigin, bluOrigin, gradientSizeRed, gradientSizeGrn, gradientSizeBlu
      ...
      return (
        (redOrigin + gradientSizeRed * fraction).toFixed(0) +
        ',' +
...
ddimgtooltip = {
  ...
  createtip: function ($, tipid, tipinfo) {
    if ($('#' + tipid).length === 0) {
      ...
      return $('<div id="' + tipid + '" class="ddimgtooltip" />')
        .html(
          (tipinfo[1]
            ? '<div class="tipinfo" id="' + tipid + '_txt">' + tipinfo[1] + '</div>'
            : '') +
            (tipinfo[0] !== null
              ? '<div style="text-align:center"><img class="tipimg" id="' +
                tipid +
                '_img" src="' +
                tipinfo[0] +
                '" /></div>'
  ...
  positiontooltip: function ($, $tooltip, e) {
    ...
    x =
      x + tipw > $(document).scrollLeft() + wWidth
        ? x - tipw - ddimgtooltip.tooltipoffsets[0] * 2
        : x
  ...
  delaybox: function ($, $tooltip) {
    if (this.showTips) {
      ddimgtooltip.delayTimer = setTimeout(function () {
